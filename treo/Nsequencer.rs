...
pub extern "C" fn reors_generated_proto_create() -> CProtoHandle {
    reo_rs::to_c_proto(proto_protocol1_build_rust::<isize,isize,isize,isize,isize>())
}

pub fn proto_protocol1_build_rust<T0, T1, T2, T3, T4>(
) -> ProtoHandle
where
    T0: 'static + Send + Sync + Sized,
    T1: 'static + Send + Sync + Sized + core::str::FromStr,
    T2: 'static + Send + Sync + Sized,
    T3: 'static + Send + Sync + Sized,
    T4: 'static + Send + Sync + Sized,
{
    let name_defs = hashmap!{
        "p_2" => Port { is_putter:  true, type_info: TypeInfo::of::<T4>() },
        "p_1" => Port { is_putter:  true, type_info: TypeInfo::of::<T2>() },
        "p_4" => Port { is_putter:  true, type_info: TypeInfo::of::<T0>() },
        "p_3" => Port { is_putter:  true, type_info: TypeInfo::of::<T3>() },
        "m1" => Mem(TypeInfo::of::<T1>()),
        "m2" => Mem(TypeInfo::of::<T1>()),
        "m3" => Mem(TypeInfo::of::<T1>()),
        "m4" => Mem(TypeInfo::of::<T1>()),
    };
    let rules = vec![
        RuleDef {
